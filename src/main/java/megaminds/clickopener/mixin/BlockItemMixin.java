...
public abstract class BlockItemMixin extends Item {
	private static final Map<String, BiFunction<World, BlockPos, NamedScreenHandlerFactory>> EASY_MAP = Map.of(
			"smithing_table", getFactoryCreaterFunc(SmithingScreenHandler::new, Text.translatable("container.upgrade")),
			"crafting_table", getFactoryCreaterFunc(CraftingScreenHandler::new, Text.translatable("container.crafting")),
			"grindstone", getFactoryCreaterFunc(GrindstoneScreenHandler::new, Text.translatable("container.grindstone_title")),
			"stonecutter", getFactoryCreaterFunc(StonecutterScreenHandler::new, Text.translatable("container.stonecutter")),
			"cartography_table", getFactoryCreaterFunc(CartographyTableScreenHandler::new, Text.translatable("container.cartography_table")),
			"loom", getFactoryCreaterFunc(LoomScreenHandler::new, Text.translatable("container.loom"))
			);
	...
	public boolean onClicked(ItemStack clickedStack, ItemStack cursorStack, Slot slot, ClickType clickType, PlayerEntity player, StackReference cursorStackReference) {
		...
		if (!player.world.isClient && Config.getInstance().clickType==clickType && Config.getInstance().isAllowed(name=Registry.ITEM.getId(this).getPath())) {
			...
			if (this == Items.ENDER_CHEST) {
				...
				factory = new SimpleNamedScreenHandlerFactory((syncId, inventory, player2)->new AnvilScreenHandler2(syncId, inventory, getContext(p.getWorld(), p.getBlockPos()), slot.inventory, slot.getIndex()), Text.translatable("container.repair"));
			}
