...
public class ReinforcedShulkersCompat {
	...
	public static void register(BiConsumer<BlockItem, ItemScreenOpener> registryFunc) {
		ModItems.REINFORCED_SHULKER_BOX_MAP.entrySet().forEach(e->{
			...
			var handler = ItemScreenOpener.requireSingleStack((ScreenFactoryOpener)(i,p,s)->new SimpleNamedScreenHandlerFactory((syncId, inventory, player)->ReinforcedStorageScreenHandler.createShulkerBoxScreen(material, syncId, inventory, new ShulkerInventory(i, material.getSize(), entityType)), name));
